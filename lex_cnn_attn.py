	...
	def train(self,s1,s2,sim_scr,itr):
		...
		for j in range(0,itr):
			...
			for i in range(0,l):
				...
				print "******************"
				if ((i+1)%10 == 0):
					fname = 'states_lex_comp_dec/lex_cnn_attn.epoch%d' %(j)
					self.save_params(fname,j)

s1_tr,s2_tr,sim_tr = process_input(data)

lcd = LCD("max","orthogonal",500,300)
#lcd.load_state('states_lex_comp_dec/lex_cnn_attn.epoch1')
...
for m in range(0,3):
	...
	file_specs = 'states_lex_comp_dec/specs_cnn_attn.txt'
